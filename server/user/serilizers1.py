...
class FullClientSerializer(DynamicFieldsMixin, serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = ['id', 'email', 'username', 'first_name', 'last_name',  'password', 'password2', 'client']
        read_only_fields = ('id',)
    ...
    def update(self):
      ...
      client.start_date = client_data.get('start_date', client.start_date)
      client.end_date = client_data.get('end_date', client.end_date)
      client.phone_number = client_data.get('phone_number', client.phone_number)
      client.establishment = client_data.get('establishment', client.establishment)
      client.save()
      return instance
...
class ClientUpdateSerializer(serializers.ModelSerializer):
  ...
  class ClientUSerializer(DynamicFieldsMixin, serializers.ModelSerializer):
...
class FullDevSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = ['id', 'email', 'username', 'first_name', 'last_name',  'password', 'password2', 'dev']
        read_only_fields = ('id', )
        extract_kwargs = {
          'password': {'write_only': True}
        }
